function exp_array=expand_array(nodef_x,nodef_y,nodef_z,node_x,node_y,node_z,path_cost,xTarget,yTarget,zTarget,CLOSED,MAX_X,MAX_Y,MAX_Z)
    ...
    if (k_x == 0.5&&k_y == 0)||(k_x == 0&&k_y == 0)
            ...
            for j= 0.5:-0.5:-0.5
                for z=0.5:-0.5:-0.5
                    if (k~=j || k~=0)  %The node itself is not its successor
                        ...
                        s_y = node_y+j;
                        s_z = node_z+z;
                        if( (s_x >-4 && s_x <=MAX_X) && (s_y >-5 && s_y <=MAX_Y) && (s_z >0 && s_z <=MAX_Z)) % node within array bound
                            disp('===============z1_2============');
        ...
        %                     end
                            if (flag == 1)
                                ...
                                exp_array(exp_count,4) = 1 * path_cost + 1 * distanced(node_x,node_y,node_z,s_x,s_y,s_z); %+ 10 * h_ThreatenW + 8 * h_value;%g(n)cost of travelling to node
                                %exp_array(exp_count,4) = distance(xTarget,yTarget,s_x,s_y);%distance between node and goal
                                exp_array(exp_count,5) = 1 * (abs(xTarget - s_x)+abs(yTarget - s_y)+abs(zTarget - s_z));%h(n)
...
%%%%%%%%%%%%状况2    
    if (k_x == 0.5&&k_y == -0.5)
            ...
            for j= 0:-0.5:-0.5
                for z=0.5:-0.5:-0.5
                    if (k~=j || k~=0)  %The node itself is not its successor
                        ...
                        s_y = node_y+j;
                        s_z = node_z+z;
                        if( (s_x >-5 && s_x <=MAX_X) && (s_y >-4 && s_y <=MAX_Y) && (s_z >0 && s_z <=MAX_Z))%node within array bound
                            disp('===============z2_2============');
                            flag=1;                     %是否在CLOSED列表标识为                    
                            for c1=1:c2
                                if(s_x == CLOSED(c1,1) && s_y == CLOSED(c1,2) && s_z == CLOSED(c1,3))
        ...
        %                         end                            
        %                         h_value = 1/s_z;
                                exp_array(exp_count,4) = 1 * path_cost + 1 * distanced(node_x,node_y,node_z,s_x,s_y,s_z); %+ 10 * h_ThreatenW + 8 * h_value;%cost of travelling to node
                                %exp_array(exp_count,4) = distance(xTarget,yTarget,s_x,s_y);%distance between node and goal
                                exp_array(exp_count,5) = 1 * (abs(xTarget - s_x)+abs(yTarget - s_y)+abs(zTarget - s_z));%fn
                                exp_array(exp_count,6) = exp_array(exp_count,4)+exp_array(exp_count,5);%fn
                                exp_count=exp_count+1;
                            end%Populate the exp_array list!!!
                        end% End of node within array bound
                    end %End of for-end loop
                end%End of z for loop
            end%End of j for loop
        end%End of k for loop
    end
